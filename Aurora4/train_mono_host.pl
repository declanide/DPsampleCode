...
unless(-e $hmmtxt){
	mkpath($hmmtxt)||die "can not rmtree ".$hmmtxt." ".$!;
}
...
mlf_path2wild($ALIGNED2,$LIB."/aligned2temp_aligned2temp.mlf");
system $AUEXE."/train_iter_host.pl $HMM_ROOT hmm".$lasthmm."_text hmm".$newhmm." $MONOLIST1 $LIB/aligned2temp_aligned2temp.mlf 3";
my $beghmm = $newhmm;
my $tohmm  = $beghmm + 1;
for(0..2){
	system $AUEXE."/train_iter_host.pl $HMM_ROOT hmm$beghmm hmm$tohmm $MONOLIST1 $LIB/aligned2temp_aligned2temp.mlf 3";
	++$beghmm;
	++$tohmm;
	#pause($_);
}
...
if(-e $LIB."/aligned2temp_aligned2temp.mlf"){
	unlink($LIB."/aligned2temp_aligned2temp.mlf")||die "can not unlink ".$LIB."/aligned2temp_aligned2temp.mlf ".$!;
}
