...
for(0..$tn){
	...
	my $traincmd = $AUEXE."/train_iter_host.pl $mainhmm hmm".$beghmm." hmm".$tohmm." $MONOLIST0 $PHONES0 3 text $TRAINLIST_FEA_HOSTPATH";
	#pause($traincmd);
	system $traincmd;
	++$beghmm;
	++$tohmm;
...
if($type == 2){
	for(0..3){
		...
		system $AUEXE."/train_iter_host.pl $mainhmm hmm".$beghmm." hmm".$tohmm." $MONOLIST1 $PHONES1 3 text $TRAINLIST_FEA_HOSTPATH";
	}
}
